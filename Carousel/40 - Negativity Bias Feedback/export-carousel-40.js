...
const CAROUSEL_HTML = path.resolve(__dirname, 'Neuro Daily Carousel - Negativity Bias Feedback.html');
const OUT_DIR       = path.resolve(__dirname, 'Exports');
const CHROME        = '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';

const IG_W   = 1080;
const VP_W   = 440;
const DPR    = IG_W / VP_W; // ~2.454
const SLIDES = 7;
...
async function main() {
  ...
  for (let i = 0; i < SLIDES; i++) {
    ...
    console.log(`✓ ${filename}  (${IG_W}×1350px)`);
  }

  await browser.close();
  console.log(`\nDone! ${SLIDES} slides → ${OUT_DIR}`);
}
